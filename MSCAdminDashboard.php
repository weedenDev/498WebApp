...
<html>
  ...
  <body>
      ...
      <form  id="regularField" name='search'  method='post'><br><br>
            ...
            <input type="text" pattern="[^'\x22><]+" title="No Special Characters (Quotes or Angle Brackets)" id="searchbar" size="80" value="<?php echo isset($_POST['searchbar']) ? $_POST['searchbar'] : '' ?>" height="60"></input><br><br>
...
if(isset($_POST['searchbutton'])){
    ...
    if ($options == "StudentID" || $options == "GraduatingYear" || $options == "FName" || $options == "Program"|| $options == "LName" || $options == "HomeProvince" || $options == "QueensEmail" || $options == "Citizenship" || $options == "Gender")
    {
    	...
    	if(empty($searchCriteria)){
    		$query = "SELECT Student.StudentID, FName, LName, Gender, GraduatingYear, Student.Program, HomeProvince, Citizenship, QueensEmail, OtherEmail, Source, Amount, StartDate, EndDate, UGProgram, FurtherStudies, Employment FROM Student LEFT JOIN Education ON Student.StudentID = Education.StudentID LEFT JOIN Funding On Student.StudentID = Funding.StudentID LEFT JOIN Future On Student.StudentID = Future.StudentID";
            if($options!="Program"){
            ...
            if($options=="StudentID"){
                $query = "SELECT Student.StudentID, FName, LName, Gender, GraduatingYear, Student.Program, HomeProvince, Citizenship, QueensEmail, OtherEmail, Source, Amount, StartDate, EndDate, UGProgram, FurtherStudies, Employment FROM Student LEFT JOIN Education ON Student.StudentID = Education.StudentID LEFT JOIN Funding On Student.StudentID = Funding.StudentID LEFT JOIN Future On Student.StudentID = Future.StudentID WHERE Student.StudentID LIKE '%$searchCriteria%'";
                if($options!="Program"){
                $query .= " AND (" . $querycondition . " )";
                }
            }
            elseif($options=="Gender"){
                $query = "SELECT Student.StudentID, FName, LName, Gender, GraduatingYear, Student.Program, HomeProvince, Citizenship, QueensEmail, OtherEmail, Source, Amount, StartDate, EndDate, UGProgram, FurtherStudies, Employment FROM Student LEFT JOIN Education ON Student.StudentID = Education.StudentID LEFT JOIN Funding On Student.StudentID = Funding.StudentID LEFT JOIN Future On Student.StudentID = Future.StudentID WHERE $options LIKE '$searchCriteria%'";
                if($options!="Program"){
                $query .= " AND (" . $querycondition . " )";
                }
            }
	...
	$stmt = $con->prepare($query);
       ...
       $stmt->execute();
        ...
        if($showStudentTable == True){
        	...
        	echo "<thead>";
        	echo " <tr> <th style='min-width:100px'> Student # </th> <th style='min-width:150px'> First Name </th> <th style='min-width:150px'> Last Name </th> <th> Gender </th> <th style='min-width:150px'> Graduating Year </th>   <th style='min-width:100px'> Program </th> <th style='min-width:100px'> Home Province  </th> <th style='min-width:100px'> Citizenship </th>  <th style='min-width:150px'> Queen's Email </th>  <th style='min-width:150px'> Alternate Email</th>  <th style='min-width:100px'> Funding Source </th>  <th style='min-width:100px'> Funding Amount </th>  <th style='min-width:225px'> Undergraduate Program </th> <th style='min-width:150px'> UG Start Date </th> <th style='min-width:150px'> UG End Date </th> <th style='min-width:150px'> Further Studies</th> <th style='min-width:150px'> Current Employment </th> <th> Edit Row </th>     </tr> </thead>";
            echo "<tbody>";
        	 while($row = $result->fetch_assoc()){
                    echo "<tr > <td id='".$row['StudentID']."' >" . $row['StudentID'] . "</td> <td> " .  $row['FName'] . "</td> <td> " . $row['LName'] .  " </td> <td> " . $row['Gender'] . "</td> <td> " .  $row['GraduatingYear'] .  "</td>  <td> " .  $row['Program'] .  "</td> <td> " .  $row['HomeProvince'] .  "</td> <td> " .  $row['Citizenship'] .  "</td> <td> " .  $row['QueensEmail'] .  "</td> <td> " .  $row['OtherEmail'] .  "</td> <td> " .  $row['Source'] .  "</td> <td> " .  $row['Amount'] .  "</td>  <td> " .  $row['UGProgram'] .  "</td> <td> " .  $row['StartDate'] .  "</td> <td> " .  $row['EndDate'] .  "</td> <td> " .  $row['FurtherStudies'] .  "</td> <td> " .  $row['Employment'] .  "</td> <td> <form method='post' action='adminCreateStudent.php'> <input type='hidden' name='studentrow' value=".$row['StudentID']."> <button type='submit' name='studentedit'>  Edit " . "</button> </form> </td>  </tr>"; 
    		}
    		echo "</tbody> </table>";
            echo "</div>";  
		...
		if($showOrganizationTable == True){
        	...
        	echo "<thead>";
        	echo " <tr> <th> OrganizationID </th> <th> Organization Name </th> <th> Organization Address</th> <th> City </th> <th> Province </th> <th> Country </th> </tr> </thead>";
        	echo "<tbody>";
             while($row = $result->fetch_assoc()){
                    echo " <tr> <td>" . $row['OrganizationID'] . "</td> <td> " . $row['OrganizationName'] . "</td> <td> " . $row['StreetNameNumber'] . " </td> <td> " . $row['City'] . "</td> <td> " . $row['Province'] .  "</td>  <td> " . $row['Country'] .  "</td> <td>  <form method='post' action='adminAddOrganization.php'> <input type='hidden' name='organizationrow' value=".$row['OrganizationID']."> <button type='submit' name='organizationedit'>  Edit " . "</button></form></td> </tr>"; 
    		}
    		echo "</tbody> </table>"; 
            echo "</div>";    
		...
		if($showPracticumTable == True){
        	...
        	echo "<thead>";
        	echo " <tr> <th style='min-width:100px'> Facility </th> <th style='min-width:100px'> Student # </th> <th style='min-width:150px'> First Name </th> <th style='min-width:150px'> Last Name </th>  <th style='min-width:300px'> Student Evaluation </th> <th> Edit Row </th> </tr> </thead>";
            echo "<tbody>";
        	 while($row = $result->fetch_assoc()){
                    echo "<tr > <td id='".$row['StudentID']."' >" . $row['Facility'] . "</td> <td> " .  $row['StudentID'] . "</td> <td> " .  $row['FName'] . "</td> <td> " . $row['LName'] .  " </td> <td> " . $row['StudentEval'] . "</td> <td>  <form method='post' action='adminAddPracMSc.php'> <input type='hidden' name='practicumrow' value=".$row['StudentID']."> <button type='submit' name='practicumedit'>  Edit " . "</button></form></td>  </tr>"; 
    		}
    		echo "</tbody> </table>";
            echo "</div>";        
        ...
        if($showThesisTable == True){
        	...
        	echo "<thead>";
        	echo " <tr> <th style='min-width:200px'> Thesis Title </th> <th style='min-width:100px'> Student # </th> <th style='min-width:150px'> First Name </th> <th style='min-width:150px'> Last Name </th>  <th style='min-width:150px'> Supervisor </th> <th style='min-width:150px'> Research Area  </th> <th style='min-width:150px'> Defense Committee </th> <th style='min-width:500px'> Abstract  </th> <th> Edit Row </th> </tr> </thead>";
            echo "<tbody>";
        	 while($row = $result->fetch_assoc()){
                    echo "<tr > <td id='".$row['StudentID']."' >" . $row['Title'] . "</td> <td> " .  $row['StudentID'] . "</td> <td> " .  $row['FName'] . "</td> <td> " . $row['LName'] .  " </td> <td> " . $row['Supervisor'] . "</td> <td> " . $row['ResearchArea'] . "</td>  <td> " . $row['DefenseCommittee'] . "</td>  <td> " . $row['Abstract'] . "</td> <td> <form method='post' action='adminCreateThesis.php'> <input type='hidden' name='thesisrow' value=".$row['StudentID']."> <button type='submit' name='thesisedit'>  Edit " . "</button></form></td>  </tr>"; 
    		}
    		echo "</tbody> </table>";
            echo "</div>";        
...
if(isset($_POST['advancedSearchButton'])){
    ...
    while($options[$i] != "blank" && $i < 4){
        ...
        if ($options[$i] == "StudentID" || $options[$i] == "GraduatingYear" || $options[$i] == "FName" || $options[$i] == "Program"|| $options[$i] == "LName" || $options[$i] == "HomeProvince" || $options[$i] == "QueensEmail" || $options[$i] == "Citizenship" || $options[$i] == "Gender")
    {
