<!DOCTYPE php>
 <?php
 /*
 File for retreiving saved views for admins
 */
// Start the session
session_start();
...
//these tables work the exact same as the ones in the advanced search
     	while($h < $i){ //first while for the headers
    ...
    while(($row = $secondresult->fetch_assoc())){ //this is filling the content
    		$t = 0;
    			echo "<tr>";
    			while($t < $i){
                    echo " <td >" . $row[$adjustedquery[$t]] . "</td>"; 
                    $t = $t +1;
    		}$s = $s +1;
    		echo "</tr>";
    	} 
